...
describe("Test fetchStats", () => {
  it("should fetch correct stats", async () => {
    let stats = await fetchStats("oskaralves");
  ...
  it("should throw error", async () => {
    ...
    await expect(fetchStats("oskaralves")).rejects.toThrow(
  ...
  it("should fetch total commits", async () => {
    mock
      .onGet("https://api.github.com/search/commits?q=author:oskaralves")
      .reply(200, { total_count: 1000 });

    let stats = await fetchStats("oskaralves", true);
    const rank = calculateRank({
      all_commits: true,
      commits: 1000,
      prs: 300,
  ...
  it("should throw specific error when include_all_commits true and API returns error", async () => {
    ...
    expect(fetchStats("oskaralves", true)).rejects.toThrow(
  ...
  it("should exclude stars of the `test-repo-1` repository", async () => {
    ...
    let stats = await fetchStats("oskaralves", true, ["test-repo-1"]);
    const rank = calculateRank({
      all_commits: true,
      commits: 1000,
      prs: 300,
  ...
  it("should fetch additional stats when it requested", async () => {
    let stats = await fetchStats("oskaralves", false, [], true, true, true);
    const rank = calculateRank({
      all_commits: false,
      commits: 100,
      prs: 300,
